...
// 🔹 MUEVE ESTA FUNCIÓN AQUÍ - ANTES DE USARLA
...
const generatePasswordResetTemplate = (name, code) => {
  ...
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>Recuperación de Contraseña MusicApp</title>
  </head>
  <body style="margin:0; padding:0; font-family: Arial, sans-serif; background:#f4f4f8;">
    <table align="center" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px; background:#ffffff; border-radius:12px; overflow:hidden;">
      <tr>
        <td style="background: linear-gradient(135deg, #ff00dd, #00d4ff); text-align:center; padding:40px 20px;">
          <h1 style="color:#ffffff; margin:0; font-size:28px;">¡Hola ${name}!</h1>
          <p style="color:#e0e7ff; font-size:16px; margin:10px 0 0;">Solicitud de recuperación de contraseña</p>
        </td>
      </tr>
      <tr>
        <td style="padding:40px 30px; text-align:center;">
          <p style="font-size:16px; color:#1e1e2f; margin-bottom:30px;">Tu código de recuperación es:</p>
          <div style="font-size:32px; font-weight:bold; color:#ff00dd; background:#ffe0f7; display:inline-block; padding:15px 30px; border-radius:8px; letter-spacing:4px;">
            ${code}
          </div>
          <p style="font-size:14px; color:#555; margin-top:30px;">Ingresa este código en la app para restablecer tu contraseña. Si no solicitaste este correo, ignóralo.</p>
        </td>
      </tr>
      <tr>
        <td style="background:#f4f4f8; text-align:center; padding:20px;">
...
exports.requestPasswordReset = async (req, res) => {
  ...
  try {
    ...
    await sendEmail(
      user.email,
      "Recuperación de contraseña MusicApp",
      null,
      generatePasswordResetTemplate(user.name, resetCode)
    );
